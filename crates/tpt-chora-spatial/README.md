# tpt-chora-spatial

Spatial and 3D engine for [tpt-chora](https://github.com/tpt-solutions/tpt-chora) ("The Depth"): stereoscopic rendering, foveated rendering (simulated gaze, not hardware eye-tracking), volumetric lighting, and HRTF-based spatial audio.

## Status

Part of tpt-chora's Phase 1 rendering engine. API is pre-1.0 and may change. Consumed optionally by [`tpt-chora-render`](https://crates.io/crates/tpt-chora-render) via its `spatial` feature.

See the workspace [ARCHITECTURE.md](https://github.com/tpt-solutions/tpt-chora/blob/master/ARCHITECTURE.md) for how this crate fits into the rest of tpt-chora.

## License

Licensed under either of Apache License, Version 2.0 or MIT license at your option.
